locals {
  secrets = var.access_key_secretsmanager_reference != "" ? [
    {
      # SecretsManager secret reference: "arn:aws:secretsmanager:region:accountId:secret:secretName[:jsonKey:versionStage:versionId]"
      name = "ACCESS_KEY",
      valueFrom = var.access_key_secretsmanager_reference
    }
  ] : []
